...
//Tables 得到所有的表信息
func Tables(db *sqlx.DB, ConnectID string, destTable []string, modelInit ModelInit) (tables []MysqlTable, err error) {

	var tableName string
	// 获取所有的表
	var arrTable []string
	arrTable, err = GetTableList(db)
	if err != nil {
		return
	}

	//var PrimaryFields map[string]string
	//PrimaryFields = make(map[string]string)
	// 获取所有的主键字段及类型
	for _, tableName = range arrTable {
		if len(destTable) > 0 {
			if StringArrayExists(destTable, tableName) == false {
				continue
			}
		}
